...
Phase 3 HUD chassis + Helm Extension bridge reception + empire dim lens (Capability 1) + empire base markers (Capability 2) + docked-ship indicators (Capability 3) of the empire overlay spec.

### Added

- **Docked-ship indicators in system view** (Capability 3 of empire-overlay spec) — three overlapping right-pointing filled chevrons (8px, 3px overlap) render beside any planet that has one or more docked ships in the bridge snapshot. The three-glyph stack is a fixed visual signature regardless of fleet size; hover tooltip shows the actual ship list (name, registration, status) with `+N more` truncation past 6 rows. Stack anchors just outside the planet's empire ring footprint (`pos.radius + 4 + 6 + glyph/2`) so it sits cleanly whether or not the planet is empire-owned. Ships docked at CX/station locations (`locationPlanetNaturalId === null`) are silently filtered out and deferred to the v2 status grid per `empire-overlay.md` §3. `IN_FLIGHT` ships excluded — they're not docked.
