...
- **CX systems referenced by their code in ship panels** — a ship bound for the Antares exchange reads "→ ANT", not "→ ZV-307", via `cxSystemLabel()`, matching the map (which already labels CX systems by their code). Scoped to the exchange itself, never its system's planets: flying to a **planet** shows the planet's display name regardless of system (CX systems like ZV-307 commonly hold several populated planets); flying to the **CX** (no destination planet) shows the CX code. Docked headers split the same way — the system-view CX-docked stack reads as the CX code, while the galaxy per-system aggregate (a rollup mixing planet- and CX-docked ships) keeps the system id. `formatDockedShipTooltip` takes an explicit `locationLabel` so the call site decides.
...
- **Empire dim / highlight lens** — toolbar button (tier-gated on bridge snapshot) and keyboard `E` toggle a lens that keeps empire systems bright and dims everything else on the galaxy view; same for empire planets in system view. Empty empire dims everything (intentional — matches empire-overlay.md §1). Toggling the lens on smoothly frames the empire's bounding box in the viewport via `MapRenderer.frameEmpire()` — and so does loading with the lens left enabled, once empire data arrives. The empire index recomputes after map data finishes loading, so a bridge snapshot arriving before the search index exists can't strand the lens dimming the whole galaxy. Gateway arcs and indicators are exempt from the lens — navigational chrome with its own `G` toggle. State persists to `helm-empire-dim` localStorage key. Icon: concentric rings (bright centre + bright inner ring + dim outer ring at 0.4 opacity).
...
- **Bridge diagnostic logging stripped for release.** The bring-up `console.log` chatter (`[Helm Bootstrap]` in the inline bootstrap, `[Helm Bridge]` in the reception layer) and the no-extension diagnostic timer are removed. Boundary failures stay loud: malformed snapshots/updates, protocol mismatches, and update-before-init still `warn`/`error`.
