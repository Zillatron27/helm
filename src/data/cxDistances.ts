...
const cxMeta = new Map<string, { code: string; currency: string }>();
...
export function computeCxDistances(): void {
  ...
  for (const cx of getAllCxStations()) {
    cxMeta.set(cx.ComexCode, { code: cx.ComexCode, currency: cx.CurrencyCode });
...
export function getCxDistances(systemId: string): CxDistanceEntry[] {
  ...
  for (const [code, result] of bfsResults) {
    ...
    entries.push({ code: meta.code, currency: meta.currency, jumps, viaGateway });
  }
