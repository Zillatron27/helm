...
async function boot(): Promise<void> {
  ...
  try {
    ...
    resourcePicker.setFilterCallback(async (materialIds) => {
      ...
      await renderer.setResourceConcentrationsAsync(getSystemsWithAnyResource(materialIds));
    });
    ...
    renderer.onAfterRebuild(() => {
      ...
      if (ids.length > 0) {
        renderer.setResourceConcentrationsAsync(getSystemsWithAnyResource(ids));
      }
